  ...
  useEffect(() => {
    const interval = setInterval(() => {
      setTableData(prev => prev.map(row => {
        ...
        const newPrice = Math.max(0.1, row.price + fluctuate);
        const newChg = row.chg + fluctuate;
                  ...
                  <tr key={idx} className="hover:bg-gray-50">
                    ...
                    <td className="py-4 px-4 text-right font-bold text-sm">
                      <motion.div
                        key={row.price}
                        initial={{ 
                          backgroundColor: row.up ? 'rgba(16, 185, 129, 0.15)' : 'rgba(244, 63, 94, 0.15)',
                          color: row.up ? '#059669' : '#e11d48'
                        }}
                        animate={{ 
                    ...
                    <td className="py-4 px-4 text-right">
                      <motion.div
                        ...
                        transition={{ duration: 1.2, ease: "easeOut" }}
                        className={`inline-block px-2 py-1 rounded-md text-xs font-bold ${row.up ? 'text-emerald-500' : 'text-red-500'}`}
                      >
                        {row.chg > 0 ? '+' : ''}{row.chg.toFixed(2)} ({row.pct > 0 ? '+' : ''}{row.pct.toFixed(2)}%)
                      </motion.div>
                    </td>
